...
FILE="$DIR/portools_py--$MINOR.sig"
...
CREATE EXTENSION portools_py WITH VERSION '$VERSION' CASCADE;
EOF

psql "${DB_ARGS[@]}" "$DB_NAME" -c '\dx+ portools_py' -A | grep '^function' | cut -d ' ' -f2- | sort -d > "$FILE"
