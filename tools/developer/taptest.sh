...
shift
PGFLAGS=$*
if ! VERSION=$(grep -E '^[[:space:]]*project\(PORTOOLS_PY[[:space:]]+VERSION[[:space:]]+([^[:space:];]+)' CMakeLists.txt | sed -E 's/.*VERSION[[:space:]]+([^[:space:];]+).*/\1/'); then
...
do
    ...
    echo "--------------------------"
    psql "${PGFLAGS}" -d "$PGDATABASE" -c "SELECT * FROM vrp_version();"
    popd
