...
OLD_VERSION=$(grep -Po '(?<=project\(PORTOOLS_PY VERSION )[^;]+' CMakeLists.txt)
...
perl -pi -e 's/project\(PORTOOLS_PY VERSION (.*)$/project\(PORTOOLS_PY VERSION '"${NEW_VERSION}"'/g' CMakeLists.txt
