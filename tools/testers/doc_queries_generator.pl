...
sub createTestDB {
    ...
    mysystem("$psql $connopts -c \"$encoding DROP EXTENSION IF EXISTS pgortools CASCADE\"  $DBNAME");
    mysystem("$psql $connopts -c \"$encoding CREATE EXTENSION pgortools CASCADE\"  $DBNAME");
